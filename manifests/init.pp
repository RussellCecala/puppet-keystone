...
#   Defaults to $::os_service_default
#
# [*fernet_keys*] THIS IS NOW A FACT GENERATED BY THE modules/keystone/lib/facter/fernet_keys.rb CUSTOM FACT.
...
#
# [*fernet_key_master*]
#   (Optional) Hostname (non FQDN) of the controller node that has the master
#   set of fernet key files.
#
...
# Begin TA265938 - Fernet Key distribution and rotation
    if $::hostname == $fernet_key_master {
      unless empty($fernet_keys) {
        ...
        #
        # Export fernet key file resources
        #
        ...
        # 
        # pick up missing keys incase someone changes the max_active_keys
        #
        File <<| tag == 'fernet_key' |>>
